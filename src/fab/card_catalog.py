...
def _get_dates(variations: list[str], card_sets: list[CardSet]) -> dict[str, tuple[Optional[datetime.date], Optional[datetime.date]]]:
    '''
    A helper function to compute the "dates" field of a card given its
    variations and list of card sets.
    '''
    result = {}
    for variation in variations:
        [vid, ved] = variation.split('-')[:2]
        for card_set in card_sets:
            if vid.startswith(card_set.identifier):
                if ved in card_set.dates:
                    result[variation] = copy.deepcopy(card_set.dates[ved])
                    break
    return result

def _parse_color(pitchstr: str) -> Optional[str]:
    '''
    A helper function for parsing card colors.
    '''
    stripped = pitchstr.strip()
    if stripped.isdigit():
        n = int(stripped)
        if n == 1:
            return 'Red'
...
def _parse_image_urls(inputstr: str, variations: list[str]) -> dict[str, str]:
    '''
    ...
    if not inputstr:
        return result
    try:
        for spec in [s.strip() for s in unidecode(inputstr).split(',')]:
            chunks = [c.strip() for c in spec.split(' - ')]
            if len(chunks) < 3:
                raise Exception(f'invalid image URL specification "{spec}"')
            url = chunks[0]
            identifier = chunks[1]
            edition = chunks[2]
            art_type = chunks[3] if len(chunks) == 4 else 'S'
            variation = 'UNKNOWN'
            for v in variations:
                if v.startswith(f'{identifier}-{edition}-') and v.endswith(f'-{art_type}'):
                    variation = v
                    break
            if variation == 'UNKNOWN': continue
            result[variation] = url
    except Exception as e:
...
def _parse_types(inputstr: str) -> dict[str, Optional[list[str] | str]]:
    ...
    try:
        ...
        for t in all_types:
            if t in CARD_TYPES:
                res['card_type'] = t
        if res['card_type'] == 'Token' and not 'Token' in res['types']:
...
class CardCatalog:
    ...
    def check_consistency(self) -> tuple[bool, Optional[str]]:
        '''
        Checks the consistency of the card catalog, returning information on
        whether there are any invalid `Card` objects.
        ...
        for i, card in enumerate(self.card_data):
            try:
                res = card.check_consistency()
            except Exception as e:
                return (False, f'{i}: Consistency check for card "{card.full_name}" encountered an exception - {e}')
            if res[0] == False:
                return (False, f'{i}: Consistency check for card "{card.full_name}" failed - {res[1]}')
        return (True, None)
        ...
        for entry in pre_card_data:
            try:
                ...
                card_data.append(Card(
                    ...
                    image_urls = _parse_image_urls(entry['Image URLs'], variations_data['variations']),
    ...
    def search(self, query: str) -> Optional[Card | CardList]:
        '''
        Searches for one or more cards by the specified string query.

        This method essentially attempts to perform a direct lookup of the
        card(s) first via `lookup_card()` or `lookup_cards()` and then falling
        back to calling `.filter()` on the list of all cards. The input string
        attempts to match by the following fields in order:
        ...
        Returns:
          The card or list of cards corresponding to the result of the query.
        '''
        try:
            return self.lookup_card(full_name=query)
        except: pass
        try:
            return self.lookup_card(identifier=query)
        except: pass
        try:
            return self.lookup_card(variation=query)
        except: pass
        try:
            res = self.lookup_cards(key='name', value=query)
            if len(res) == 1:
                return res[0]
            elif len(res) > 1:
                return res
        ...
        cards = self.cards()
        res = cards.filter(name=query)
        if len(res) == 1:
            return res[0]
        elif len(res) > 1:
            return res
        res = cards.filter(body=query)
        if len(res) == 1:
            return res[0]
        elif len(res) > 1:
            return res
    ...
    def to_dict(self) -> dict[str, list[dict]]:
        ...
        '''
        return {
            'card_data': [card.to_dict(convert_dates=True) for card in self.card_data],
